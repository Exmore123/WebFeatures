...
function MultiSelectTreeView() {
    return (
        ...
        >
            ...
            <TreeItem nodeId="2" label="C#">
                {GetNavLink("/multithreading", "Multithreading")}
            </TreeItem>
            <TreeItem nodeId="3" label="Login Forms">
                {GetNavLink("/glass_login", "GlassLogin")}
            </TreeItem>
            <TreeItem nodeId="4" label="Tables">
                {GetNavLink("/table_with_scroll", "Table With Scroll")}
            </TreeItem>
            <TreeItem nodeId="5" label="DropDowns">
                {GetNavLink("/custom_dropdown", "Custom Drop Down")}
                {GetNavLink("/multi_dropdown", "Multi Drop Down")}
            </TreeItem>
            <TreeItem nodeId="6" label="Just for fun">
                {GetNavLink("/tic_tac_toe", "TicTacToe")}
                {GetNavLink("/flip_cards", "Flip Cards")}
            </TreeItem>
        </TreeView>
    );
}